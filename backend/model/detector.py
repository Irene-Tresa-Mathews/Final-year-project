...
class DetectorTF2:
    ...
    def DetectFromImage(self, img):
        ...
        input_tensor = np.expand_dims(img, 0) # (1, W, H, 3)
    ...
    def DisplayDetections(self, image, boxes_list, det_time=None):
        if not boxes_list: return image, 0  # input list is empty
        img = image.copy()
        for idx in range(len(boxes_list)):
            ...
            confidence = np.round(boxes_list[idx][-1], 2)
            score = str(confidence * 100) + "%"
            ...
            if detected_class == 'mask':
                ...
                cv2.putText(img, text, (x_min + 5, y_min - 7), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 3)
            else:
                cv2.rectangle(img, (x_min, y_min), (x_max, y_max), (0, 0, 255), 3)
                cv2.putText(img, text, (x_min + 5, y_min - 7), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 3)

        if det_time != None:
            fps = round(1000. / det_time, 1)
            fps_txt = str(fps) + " FPS"
            cv2.putText(img, fps_txt, (25, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2)
        return img, confidence*100